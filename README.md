...
In `oauth` mode the server acts as the MCP authorization server for clients such as Claude Web. It exposes local `/.well-known/oauth-authorization-server`, `/register`, `/authorize`, `/token`, and `/.well-known/oauth-protected-resource/mcp` endpoints, brokers the upstream authorization-code exchange, and enforces bearer tokens on `POST /mcp`. If Cloudflare Access injects `Cf-Access-Jwt-Assertion`, the server will still treat it like a bearer token automatically.
...
The bridge now advertises itself as the MCP authorization server from `/.well-known/oauth-authorization-server`, dynamically registers MCP clients at `/register`, starts brokered sign-in at `/authorize`, exchanges the upstream code at the callback URL, and issues MCP bearer tokens from `/token`.
