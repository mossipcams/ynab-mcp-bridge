import { z } from "zod";
import { buildAccountSnapshotSummary, buildAssignedSpentSummary, compactObject, formatMilliunits, toSpentMilliunits, toTopRollups, } from "./financeToolUtils.js";
...
export async function execute(input, api) {
    try {
        ...
        const result = await withResolvedPlan(input.planId, api, async (planId) => {
            ...
            const { activeAccounts, positiveAccounts, negativeAccounts, netWorthMilliunits, liquidCashMilliunits, onBudgetAccountCount, } = buildAccountSnapshotSummary(accountsResponse.data.accounts);
