...
export async function execute(input, api) {
    try {
        ...
        return await withResolvedPlan(input.planId, api, async (planId) => {
            ...
            const monthCategoryBudgetById = new Map(monthResponse?.data.month.categories
                .filter((category) => !category.deleted)
                .map((category) => [category.id, category.budgeted ?? 0]) ?? []);
            ...
            if (format === "prose") {
                return toProseResult(buildProse(`Spending Summary (${payload.from_month} to ${payload.to_month})`, [
