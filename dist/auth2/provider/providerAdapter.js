...
function logUpstreamTokenEvent(event, details) {
    logAppEvent("oauth", event, {
        ...getRequestLogFields(),
        ...details,
    });
}
function getUpstreamUrlDetails(url) {
...
function getTokenRequestDetails(body, url, grantType) {
    return {
        grantType,
...
function getTokenResponseDetails(tokens, status, url) {
    return {
...
async function exchangeTokens(fetchFn, url, body, failureMessage, grantType) {
...
export function createProviderAdapter(config, fetchFn) {
    return {
