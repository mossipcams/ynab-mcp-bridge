...
async function waitForLocalBridge(url, timeoutMs = 5_000) {
...
async function startLocalBridge(argv) {
    ...
    const child = spawn(process.execPath, [new URL("./index.js", import.meta.url).pathname, "--transport", "http"], {
        env: {
            ...process.env,
            MCP_HOST: options.host,
            MCP_PATH: options.path,
            MCP_PORT: String(port),
            YNAB_API_TOKEN: process.env["YNAB_API_TOKEN"] ?? "reliability-test-token",
        },
        stdio: "ignore",
    });
    try {
...
async function main() {
    ...
    try {
        process.exitCode = await executeReliabilityHttpCli(startedBridge
