...
export const description =
  "Returns a compact assigned, spent, and available trend for a category or category group across months.";
...
export async function execute(
  input: { planId?: string; fromMonth?: string; toMonth?: string; categoryId?: string; categoryGroupName?: string },
  api: ynab.API,
) {
  try {
    ...
    return await withResolvedPlan(input.planId, api, async (planId) => {
      ...
      const peakPeriod = periods.reduce((peak, period) => (
