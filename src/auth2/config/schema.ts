...
function fingerprintRedirectUri(value: string) {
  return createHash("sha256").update(value).digest("hex").slice(0, 12);
}

function assertUniqueClientIds(clients: AuthConfig["clients"]) {
...
export function createAuthStartupLogDetails(config: AuthConfig) {
  return {
    ...
    redirectUriFingerprints: Object.fromEntries(
      config.clients.map((client) => [client.clientId, fingerprintRedirectUri(client.redirectUri)]),
    ),
    scopesByClient: Object.fromEntries(
      config.clients.map((client) => [client.clientId, [...client.scopes]]),
    ),
