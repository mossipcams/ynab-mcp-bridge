...
describe("financial diagnostics tools", () => {
  ...
  it("expands recurring scheduled outflows before evaluating cash shortfall risk", async () => {
    const api = {
      accounts: {
        getAccounts: vi.fn().mockResolvedValue({
          data: {
            accounts: [
              { id: "acct-1", name: "Checking", on_budget: true, deleted: false, closed: false, balance: 300000 },
            ],
          },
        }),
      },
    ...
    expect(parseText(result as any)).toEqual(expect.objectContaining({
      ...
      metrics: expect.objectContaining({
        upcoming_30d_net: "-500.00",
      }),
      top_risks: expect.arrayContaining([
        { code: "cash_shortfall", severity: "high" },
      ]),
    }));
  });
  ...
  it("excludes transfer transactions from financial health cleanup metrics", async () => {
    const api = {
      accounts: {
        getAccounts: vi.fn().mockResolvedValue({
          data: {
  ...
  it("does not flag credit card payment categories as spending anomalies", async () => {
    const api = {
      months: {
        getPlanMonth: vi
          .fn()
