...
function addAccountToSnapshotSummary<TAccount extends AccountBalanceLike>(
  summary: AccountSnapshotSummary<TAccount>,
  account: TAccount,
) {
...
function buildActiveAccountBalances(accounts: Array<AccountBalanceLike & { id: string }>) {
  return new Map(accounts
    .filter((account) => !account.deleted)
    .map((account) => [account.id, account.balance] as const));
}

function buildDescendingTrackedTransactions(transactions: AccountTransactionLike[], balances: Map<string, number>) {
  return transactions
...
function applyTransactionsAfterMonthEnd(
  balances: Map<string, number>,
  transactionsDescending: AccountTransactionLike[],
  monthEnd: string,
...
function buildHistoricalSnapshot<TAccount extends AccountBalanceLike & { id: string }>(
  accounts: TAccount[],
  balances: Map<string, number>,
) {
  return accounts
...
>(
  ...
  for (const month of sortedMonths) {
    ...
    transactionIndex = applyTransactionsAfterMonthEnd(
      balances,
      transactionsDescending,
      monthEnd,
      transactionIndex,
    );
