...
export const description =
  "Returns a compact month-by-month net worth trajectory with liquid cash and debt across a month range.";
...
function isIncludedAccount(account: ynab.Account) {
...
export async function execute(
  ...
  try {
    ...
    return await withResolvedPlan(input.planId, api, async (planId) => {
      ...
      const balancesByMonth = reconstructHistoricalAccountBalances(
        accounts,
      ...
      return toTextResult({
        ...
        change_net_worth: formatMilliunits(
          netWorthMilliunits(balancesByMonth.get(toMonth) ?? accounts)
