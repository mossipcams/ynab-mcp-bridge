...
export const description =
  "Returns a compact 70/20/10-style income allocation summary across needs, wants, and savings or debt.";
...
export async function execute(
  input: {
    planId?: string;
    month?: string;
    needsGroupNames?: string[];
    wantsGroupNames?: string[];
    savingsDebtGroupNames?: string[];
    topN?: number;
  },
  api: ynab.API,
) {
  try {
    ...
    return await withResolvedPlan(input.planId, api, async (planId) => {
      ...
      const bucketDefinitions: Record<CategoryBucketName, { groupNames: string[]; amount: (category: ynab.Category) => number; targetPercent: number }> = {
        needs: {
