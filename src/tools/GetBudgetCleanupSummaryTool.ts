...
export const description =
  "Returns a compact cleanup punch-list for uncategorized, unapproved, uncleared, and overspent items.";
...
export async function execute(
  ...
  try {
    ...
    return await withResolvedPlan(input.planId, api, async (planId) => {
      ...
      const hiddenProblemCategories = monthResponse.data.month.categories.filter(
        (category) => !category.deleted && category.hidden && (category.balance < 0 || (category.goal_under_funded ?? 0) > 0),
      );
